...
from core.stock_replay import (
    build_stock_replay_dataset,
...
if page == "Research Overview":
    ...
    st.caption(
        "Uses only completed 09:15–10:45 candles available at the 11:15 decision gate. "
...
elif page == "Stock Session Replay":
    st.subheader("Point-in-Time Stock Session Replay & Casebook")
    st.caption(
        "Freezes the research screen at 11:15. Forward returns stay hidden until Reveal is enabled, "
        "preventing hindsight from contaminating the decision review."
    )
    if fno_uploaded is None:
        st.info("Upload the ALL F&O research CSV in the sidebar.")
    else:
        prepared,outcomes,_,_,replay_problems=load_fno_research(fno_uploaded.getvalue())
        signals,signal_problems=load_phase3c_signals(fno_uploaded.getvalue())
        problems=replay_problems+signal_problems
        if problems:
            st.error(" | ".join(problems))
        else:
            with st.spinner("Building point-in-time replay casebook..."):
                auction_data=build_morning_auction_features(prepared,outcomes)
                replay_data=build_stock_replay_dataset(signals,auction_data)
            replay_dates=sorted(replay_data["session_date"].unique(),reverse=True)
            r1,r2,r3=st.columns(3)
            with r1:
                replay_date=st.selectbox("Replay session",replay_dates)
            with r2:
                replay_checkpoint=st.selectbox("Outcome checkpoint",["11:45","13:15","15:15"],index=2)
            with r3:
                reveal=st.toggle("Reveal future outcomes",value=False)
            session_view=replay_session(replay_data,replay_date,replay_checkpoint,reveal)
            if not reveal:
                st.info("DECISION MODE: all post-11:15 returns are hidden.")
            else:
                st.warning(f"REVIEW MODE: {replay_checkpoint} outcomes are now visible.")
            st.dataframe(session_view,use_container_width=True,hide_index=True)
            st.markdown("#### Sector breadth known at 11:15")
            st.dataframe(replay_sector_breadth(replay_data,replay_date),use_container_width=True,hide_index=True)
            st.download_button(
